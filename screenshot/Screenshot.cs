...
namespace screenshot
{
	public partial class Screenshot : Form
	{
		...
		private TransparentScreen[] scrn;
		...
		public Screenshot()
		{
			...
			try
			{
				...
				settings = File.ReadAllLines(path_settings);
		...
		public void dispose_form(Point scrn_pos, Point one_, Point two_)
		{
			...
			foreach (TransparentScreen scrn_ in scrn)
			{
				scrn_.Dispose();
			}
		...
		private void form_transperent()
		{
			...
			scrn = new TransparentScreen[a];
			for (int i = 0; i < a; i++)
			{
				scrn[i] = new TransparentScreen(Screen.AllScreens[i], this);
